...
import { createJSONEditor, JSONEditorPropsOptional } from 'vanilla-jsoneditor-cn';
import type { JsonEditor, MenuItem, IconDefinition } from 'vanilla-jsoneditor-cn';
...
const options :JSONEditorPropsOptional = {
  mode: 'tree',
  // parser
  content: { json: props.modelValue ? JSON.parse(props.modelValue) : {} },
  ...
  onChange: (updatedContent, previousContent, { contentErrors, patchResult }) => {
    if (updatedContent.json !== undefined) {
      emit('update:modelValue', updatedContent.json);
    }
  },
  onError(err: Error) {
...
const updateEditorHeight = () => {
  if (editorContainer.value) {
    ...
    const newHeight = Math.max(parseInt(props.minHeight), windowHeight - containerRect.top - 20 - 30);
...
watch(() => props.modelValue, (newValue) => {
  if (editor && JSON.stringify(newValue) !== JSON.stringify(editor.get())) {
    editor.update({ json: newValue });
  }
}, { deep: true });
</script>

<template>
  <div ref="editorContainer" class="json-editor" :class="{'jse-theme-dark': isDark}"></div>
</template>
...
.json-editor {
  ...
  .jse-menu {
    ...
    .jse-button:hover, .jse-button:active {
    ...
    .jse-button:focus{
      transition: background-color 1s, color 2s !important;
    }
