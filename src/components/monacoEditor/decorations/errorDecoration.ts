 ...
 * 如果样式已存在则不会重复添加
 */
export const addErrorLineHighlightStyles = (): HTMLStyleElement => {
  // 检查是否已存在相同类名的样式元素
  if (document.querySelector(".error-line-highlight-style")) {
    return document.querySelector(
      ".error-line-highlight-style",
    ) as HTMLStyleElement;
  }

  const styleElement = document.createElement("style");

  styleElement.className = "error-line-highlight-style";
  styleElement.textContent = `
    .errorLineHighlight {
      background-color: rgba(255, 0, 0, 0.2);
      border-left: 3px solid #ff0000;
    }
    .monaco-editor.vs-dark .errorLineHighlight,
    .monaco-editor.hc-black .errorLineHighlight {
 ...
 * 移除错误行高亮样式
 */
export const removeErrorLineHighlightStyles = (): void => {
  const styleElement = document.querySelector(".error-line-highlight-style");
